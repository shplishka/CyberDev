...
class VirustotalClient:
    def __init__(self, virustotal_url: str, api_key):
        self._virustotal_base_url = virustotal_url
        self._api_key = api_key

    def get_sub_domains(self, domain_name: str) -> List[str]:
        get_sub_domains_url = self._generate_full_api_v1_url(
            DOMAIN_REPORT_END_POINT)
        domain_request = {'apikey': self._api_key,
                          'domain': domain_name}  # TODO:shit code
        virustotal_api_response: Response = requests.get(
            get_sub_domains_url, domain_request)
